...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        ...
        result['filters'] = {}
        for tid, kws in self.sub_map.items():
            value = [{"n": "全部", "v": ""}] + [{"n": kw, "v": kw} for kw in kws]
            result['filters'][tid] = [{"key": "type", "name": "子分类", "value": value}]
        try:
        ...
        try:
            ...
            if isinstance(extend, dict):
                ...
                if val is None:
                    val = ""
                val = str(val).strip()
                if val in self.sub_map.get(str(tid), []):
                    kw = val
            if kw:
